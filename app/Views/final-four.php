<nav class="finals">
    <ul class="nav-tab">

        <li id="finals7">2007</li>
        <li id="finals8">2008</li>
        <li id="finals9">2009</li>
        <li id="finals10">2010</li>
        <li id="finals11">2011</li>
    </ul>
</nav>
<section>
    <table class="table hidden" id=tab2007>
        <tr class="semis">
            <td class="opponent opponent-border">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals7[1]->id}.png"?> alt="grb">
                <?=$finals7[1]->team?>
            </td>
            <td class="score">3</td>
            <td class="bottom-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="semis">
            <td class="opponent">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals7[4]->id}.png"?> alt="grb">
                <?=$finals7[4]->team?>
            </td>
            <td class="score">2</td>
            <td class="vertical-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="spacer">
            <td></td>
            <td></td>
            <td class="vertical-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="finals">
            <td></td>
            <td></td>
            <td class="vertical-border"></td>
            <td class="bottom-border"></td>
            <td class="opponent opponent-border">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals7[1]->id}.png"?> alt="grb">
                <?=$finals7[1]->team?>
            </td>
            <td class="score">1</td>
            <td></td>
        </tr>
        <tr class="finals">
            <td></td>
            <td></td>
            <td class="vertical-border"></td>
            <td></td>
            <td class="opponent">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals7[2]->id}.png"?> alt="grb">
                <?=$finals7[2]->team?>
            </td>
            <td class="score">0</td>
            <td></td>
        </tr>
        <tr class="spacer">
            <td></td>
            <td></td>
            <td class="vertical-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="semis">
            <td class="opponent opponent-border">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals7[2]->id}.png"?> alt="grb">
                <?=$finals7[2]->team?>
            </td>
            <td class="score">1</td>
            <td class="bottom-border vertical-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="semis">
            <td class="opponent">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals7[3]->id}.png"?> alt="grb">
                <?=$finals7[3]->team?>
            </td>
            <td class="score">0</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="playoff">
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td class="opponent opponent-border">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals7[4]->id}.png"?> alt="grb">
                <?=$finals7[4]->team?>
            </td>
            <td class="score">1(2)</td>
            <td></td>
        </tr>
        <tr class="playoff">
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td class="opponent">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals7[3]->id}.png"?> alt="grb">
                <?=$finals7[3]->team?>
            </td>
            <td class="score">1(1)</td>
            <td></td>
        </tr>
    </table>
</section>
<section>
    <table class="table hidden" id=tab2008>
        <tr class="semis">
            <td class="opponent opponent-border">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals8[1]->id}.png"?> alt="grb">
                <?=$finals8[1]->team?>
            </td>
            <td class="score">3</td>
            <td class="bottom-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="semis">
            <td class="opponent">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals8[4]->id}.png"?> alt="grb">
                <?=$finals8[4]->team?>
            </td>
            <td class="score">1</td>
            <td class="vertical-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="spacer">
            <td></td>
            <td></td>
            <td class="vertical-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="finals">
            <td></td>
            <td></td>
            <td class="vertical-border"></td>
            <td class="bottom-border"></td>
            <td class="opponent opponent-border">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals8[1]->id}.png"?> alt="grb">
                <?=$finals8[1]->team?>
            </td>
            <td class="score">1</td>
            <td></td>
        </tr>
        <tr class="finals">
            <td></td>
            <td></td>
            <td class="vertical-border"></td>
            <td></td>
            <td class="opponent">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals8[2]->id}.png"?> alt="grb">
                <?=$finals8[2]->team?>
            </td>
            <td class="score">0</td>
            <td></td>
        </tr>
        <tr class="spacer">
            <td></td>
            <td></td>
            <td class="vertical-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="semis">
            <td class="opponent opponent-border">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals8[2]->id}.png"?> alt="grb">
                <?=$finals8[2]->team?>
            </td>
            <td class="score">2</td>
            <td class="bottom-border vertical-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="semis">
            <td class="opponent">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals8[3]->id}.png"?> alt="grb">
                <?=$finals8[3]->team?>
            </td>
            <td class="score">0</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="playoff">
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td class="opponent opponent-border">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals8[4]->id}.png"?> alt="grb">
                <?=$finals8[4]->team?>
            </td>
            <td class="score">0</td>
            <td></td>
        </tr>
        <tr class="playoff">
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td class="opponent">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals8[3]->id}.png"?> alt="grb">
                <?=$finals8[3]->team?>
            </td>
            <td class="score">1</td>
            <td></td>
        </tr>
    </table>
</section>
<section>
    <table class="table hidden" id=tab2009>
        <tr class="semis">
            <td class="opponent opponent-border">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals9[1]->id}.png"?> alt="grb">
                <?=$finals9[1]->team?>
            </td>
            <td class="score">2</td>
            <td class="bottom-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="semis">
            <td class="opponent">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals9[4]->id}.png"?> alt="grb">
                <?=$finals9[4]->team?>
            </td>
            <td class="score">0</td>
            <td class="vertical-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="spacer">
            <td></td>
            <td></td>
            <td class="vertical-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="finals">
            <td></td>
            <td></td>
            <td class="vertical-border"></td>
            <td class="bottom-border"></td>
            <td class="opponent opponent-border">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals9[1]->id}.png"?> alt="grb">
                <?=$finals9[1]->team?>
            </td>
            <td class="score">4</td>
            <td></td>
        </tr>
        <tr class="finals">
            <td></td>
            <td></td>
            <td class="vertical-border"></td>
            <td></td>
            <td class="opponent">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals9[3]->id}.png"?> alt="grb">
                <?=$finals9[3]->team?>
            </td>
            <td class="score">0</td>
            <td></td>
        </tr>
        <tr class="spacer">
            <td></td>
            <td></td>
            <td class="vertical-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="semis">
            <td class="opponent opponent-border">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals9[2]->id}.png"?> alt="grb">
                <?=$finals9[2]->team?>
            </td>
            <td class="score">0</td>
            <td class="bottom-border vertical-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="semis">
            <td class="opponent">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals9[3]->id}.png"?> alt="grb">
                <?=$finals9[3]->team?>
            </td>
            <td class="score">2</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="playoff">
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td class="opponent opponent-border">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals9[4]->id}.png"?> alt="grb">
                <?=$finals9[4]->team?>
            </td>
            <td class="score">0</td>
            <td></td>
        </tr>
        <tr class="playoff">
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td class="opponent">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals9[2]->id}.png"?> alt="grb">
                <?=$finals9[2]->team?>
            </td>
            <td class="score">1</td>
            <td></td>
        </tr>
    </table>
</section>
<section>
    <table class="table hidden" id=tab2010>
        <tr class="semis">
            <td class="opponent opponent-border">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals10[1]->id}.png"?> alt="grb">
                <?=$finals10[1]->team?>
            </td>
            <td class="score">9</td>
            <td class="bottom-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="semis">
            <td class="opponent">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals10[4]->id}.png"?> alt="grb">
                <?=$finals10[4]->team?>
            </td>
            <td class="score">1</td>
            <td class="vertical-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="spacer">
            <td></td>
            <td></td>
            <td class="vertical-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="finals">
            <td></td>
            <td></td>
            <td class="vertical-border"></td>
            <td class="bottom-border"></td>
            <td class="opponent opponent-border">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals10[1]->id}.png"?> alt="grb">
                <?=$finals10[1]->team?>
            </td>
            <td class="score">2</td>
            <td></td>
        </tr>
        <tr class="finals">
            <td></td>
            <td></td>
            <td class="vertical-border"></td>
            <td></td>
            <td class="opponent">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals10[3]->id}.png"?> alt="grb">
                <?=$finals10[3]->team?>
            </td>
            <td class="score">1</td>
            <td></td>
        </tr>
        <tr class="spacer">
            <td></td>
            <td></td>
            <td class="vertical-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="semis">
            <td class="opponent opponent-border">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals10[2]->id}.png"?> alt="grb">
                <?=$finals10[2]->team?>
            </td>
            <td class="score">0(1)</td>
            <td class="bottom-border vertical-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="semis">
            <td class="opponent">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals10[3]->id}.png"?> alt="grb">
                <?=$finals10[3]->team?>
            </td>
            <td class="score">0(2)</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="playoff">
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td class="opponent opponent-border">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals10[4]->id}.png"?> alt="grb">
                <?=$finals10[4]->team?>
            </td>
            <td class="score">0</td>
            <td></td>
        </tr>
        <tr class="playoff">
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td class="opponent">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals10[2]->id}.png"?> alt="grb">
                <?=$finals10[2]->team?>
            </td>
            <td class="score">4</td>
            <td></td>
        </tr>
    </table>
</section>
<section>
    <table class="table hidden" id=tab2006>
        <tr class="semis">
            <td class="opponent opponent-border">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals11[1]->id}.png"?> alt="grb">
                <?=$finals11[1]->team?>
            </td>
            <td class="score">0</td>
            <td class="bottom-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="semis">
            <td class="opponent">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals11[4]->id}.png"?> alt="grb">
                <?=$finals11[4]->team?>
            </td>
            <td class="score">6</td>
            <td class="vertical-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="spacer">
            <td></td>
            <td></td>
            <td class="vertical-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="finals">
            <td></td>
            <td></td>
            <td class="vertical-border"></td>
            <td class="bottom-border"></td>
            <td class="opponent opponent-border">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals11[4]->id}.png"?> alt="grb">
                <?=$finals11[4]->team?>
            </td>
            <td class="score">0</td>
            <td></td>
        </tr>
        <tr class="finals">
            <td></td>
            <td></td>
            <td class="vertical-border"></td>
            <td></td>
            <td class="opponent">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals11[2]->id}.png"?> alt="grb">
                <?=$finals11[2]->team?>
            </td>
            <td class="score">1</td>
            <td></td>
        </tr>
        <tr class="spacer">
            <td></td>
            <td></td>
            <td class="vertical-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="semis">
            <td class="opponent opponent-border">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals11[2]->id}.png"?> alt="grb">
                <?=$finals11[2]->team?>
            </td>
            <td class="score">1(3)</td>
            <td class="bottom-border vertical-border"></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="semis">
            <td class="opponent">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals11[3]->id}.png"?> alt="grb">
                <?=$finals11[3]->team?>
            </td>
            <td class="score">1(2)</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="playoff">
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td class="opponent opponent-border">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals11[1]->id}.png"?> alt="grb">
                <?=$finals11[1]->team?>
            </td>
            <td class="score">4</td>
            <td></td>
        </tr>
        <tr class="playoff">
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td class="opponent">
                <img class="club-logo__small" src=<?="/public/images/logos/{$finals11[3]->id}.png"?> alt="grb">
                <?=$finals11[3]->team?>
            </td>
            <td class="score">3</td>
            <td></td>
        </tr>
    </table>
</section>
<hr>
<section>
    <table class="standings">
        <thead class="background-row">
            <tr>
                <th colspan="2">zbirna tabela</th>
                <th>ut</th>
                <th>pob</th>
                <th>ner</th>
                <th>izg</th>
                <th class="goals">+</th>
                <th class="goals">-</th>
                <th>bod</th>
            </tr>
        </thead>
        <tbody>
            <?php
foreach ($combinedTable as $i => $row): ?>
            <tr>
                <td><?=++$i?></td>
                <td class="standings-club__width">
                    <a href=<?="/ekipa/$row->id"?>>
                        <img class="club-logo__small" src=<?="/public/images/logos/$row->id.png"?> alt="grb">
                        <?=$row->team_name?>
                    </a>
                </td>
                <td class="text-align__center standings-cell__width"><?=$row->gamesAll?></td>
                <td class="text-align__center standings-cell__width"><?=$row->gamesWon?></td>
                <td class="text-align__center standings-cell__width"><?=$row->gamesDrew?></td>
                <td class="text-align__center standings-cell__width"><?=$row->gamesLost?></td>
                <td class="goals text-align__center standings-cell__width"><?=$row->goalsFor?></td>
                <td class="goals text-align__center standings-cell__width"><?=$row->goalsAgg?></td>
                <td class="text-align__center text-bold standings-cell__width"><?=$row->pointsAll?></td>
            </tr>
            <?php endforeach?>
        </tbody>
        <tfoot class="background-row">
            <tr>
                <th colspan="9">Fair Play Liga Budućih Šampiona</th>
            </tr>
        </tfoot>
    </table>
</section>
<script>
let nav = document.querySelectorAll('.finals li')
nav[0].className = 'nav-select'
document.querySelector('#tab2006').style.display = 'block'

for (let i = 0; i < nav.length; i++) {
    nav[i].addEventListener('mouseup', navigation)
}

function navigation() {
    for (n of nav) {
        n.classList.remove('nav-select')
    }
    this.className = 'nav-select'
    let id = this.textContent
    let tables = document.querySelectorAll('.table')
    for (let i = 0; i < tables.length; i++) {
        tables[i].style.display = 'none'
    }
    document.querySelector('#tab' + id).style.display = 'block'
}
</script>